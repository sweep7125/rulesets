...
def _is_valid_hostname_ascii(puny: str) -> bool:
    if not puny or len(puny) > 253: return False
    for lbl in puny.split("."):
        if not (1 <= len(lbl) <= 63): return False
        if _LDH_LABEL_RE.fullmatch(lbl) is None: return False
    return True
...
def parse_xray_file_recursive(path: str, visited: Set[str]) -> Tuple[Set[str], Set[str]]:
    ...
    if abspath in visited: return set(), set()
...
def _trie_insert(root: _TrieNode, base: str, flag: str) -> None:
    ...
    if flag == "suffix": node.suffix = True
    elif flag == "full": node.full = True

def _prune(node: _TrieNode, has_suffix_above: bool = False) -> None:
    if has_suffix_above:
        node.suffix = False; node.full = False
        for ch in node.children.values(): _prune(ch, True)
        return
    if node.suffix and node.full: node.full = False
    next_has_suffix = has_suffix_above or node.suffix
    for ch in node.children.values(): _prune(ch, next_has_suffix)
...
def _trie_collect(node: _TrieNode, path_rev: List[str], out_suffix: Set[str], out_full: Set[str]) -> None:
    ...
    if node.suffix and base: out_suffix.add(base)
    if node.full and base: out_full.add(base)
...
def optimize_suffix_full(entries: Iterable[Tuple[str,str]]) -> Tuple[Set[str], Set[str]]:
    root=_TrieNode()
    for t,b in entries:
        if b: _trie_insert(root,b,t)
    _prune(root, False)
    S:set[str]=set(); E:set[str]=set()
...
def intersect_pair(A_suffix: Set[str], A_full: Set[str],
    ...
    # суффикс×суффикс: итерируемся по меньшему набору
    ...
    # full из A, которые попадают в B
    for fr in A_full:
        if fr in B_full or any(is_under(fr, sc) for sc in B_suffix):
            out_full.add(fr)

    # full из B, которые попадают в A
    for fc in B_full:
...
def intersect_many(groups: List[Tuple[Set[str], Set[str]]]) -> Tuple[Set[str], Set[str]]:
    ...
    for s2, e2 in groups[1:]:
        if not S and not E:
            break  # ранний выход: дальше уже пусто
...
def render_xray(suffixes: Set[str], fulls: Set[str]) -> List[str]:
    ...
    def label_count(d: str) -> int: return d.count(".")+1 if d else 0
    items.sort(key=lambda t: (rank[t[0]], label_count(t[1]), t[1]))
    out=[]
    for k,b in items:
        if k=="full": out.append(f"full:{b}")
        else: out.append(b)  # голая строка = XRAY domain:
    return out
...
def read_group(arg: str) -> Tuple[Set[str], Set[str]]:
    ...
    for p in [s.strip() for s in arg.split(",") if s.strip()]:
        ...
        suffix |= s
        full |= f
    # предоптимизация группы снижает объем парных проверок
...
def main():
    ...
    if args.mode != "intersect":
        print("unsupported mode", file=sys.stderr); sys.exit(2)
    if len(args.set) < 2:
    ...
    with open(dst, "w", encoding="utf-8") as f:
        for line in render_xray(S,E): f.write(line+"\n")
